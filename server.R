shinyServer(function(input, output){
  ...
  #This creates a line chart of the closing prices, not super useful
  # output$price_plot <- renderPlot({
  #   prices_input <- all_prices %>% filter(., ticker== input$selected) %>% 
  #     select(., ticker, date, price.close)
  #   price_data <- prices_input %>% slice(1:input$length)
  #   ggplot(data = price_data, aes(x=date, y= price.close)) +
  #     geom_line()
  # })
